...
def criar_graficos_comparativos(arquivos):
    ...
    fig.suptitle('Comparativo de Performance entre Bibliotecas', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('Desempenho de FPS (Quadros por Segundo)')
    ax1.set_ylabel('FPS')
    ...
    ax4.set_title('Total de Anomalias Detectadas')
    ...
    # Criar gráfico de radar para comparação geral
    criar_grafico_radar(dados, bibliotecas)

def criar_grafico_radar(dados, bibliotecas):
    """
    Cria um gráfico radar para comparação geral
    """
    ...
    def normalizar(valor, max_val):
        return valor / max_val if max_val > 0 else 0
    
    # Calcular valores máximos para normalização
    max_fps = max([dados[bib].get('fps_media', 0) for bib in bibliotecas]) or 1
    max_memory = max([dados[bib].get('memory_media', 0) for bib in bibliotecas]) or 1
    max_rendertime = max([dados[bib].get('rendertime_media', 0) for bib in bibliotecas]) or 1
    max_anomalias = max([(dados[bib].get('anomalias_fps', 0) + 
                         dados[bib].get('anomalias_memory', 0) + 
                         dados[bib].get('anomalias_rendertime', 0)) for bib in bibliotecas]) or 1
    
    # Categorias (invertendo rendertime e anomalias para que maior seja melhor)
    categorias = ['FPS', 'Memória', 'Render Time', 'Estabilidade']
    N = len(categorias)
    ...
    for i, bib in enumerate(bibliotecas):
        valores = [
            normalizar(dados[bib].get('fps_media', 0), max_fps),
            normalizar(1/dados[bib].get('memory_media', 1), 1/max_memory if max_memory > 0 else 1),
            normalizar(1/dados[bib].get('rendertime_media', 1), 1/max_rendertime if max_rendertime > 0 else 1),
            normalizar(1 - (dados[bib].get('anomalias_fps', 0) + 
                           dados[bib].get('anomalias_memory', 0) + 
                           dados[bib].get('anomalias_rendertime', 0)) / max_anomalias, 1)
        ]
        
        valores += valores[:1]
        ax.plot(angles, valores, linewidth=2, linestyle='solid', label=bib, color=cores[i])
    ...
    ax.set_title('Comparativo Geral de Performance (Normalizado)\nMaior é melhor em todas as categorias', 
                size=14, pad=20)
...
arquivos_para_comparar = ['threejs_benchmark.txt', 'pixi_benchmark.txt', 'd3_benchmark.txt', 'p5js_benchmark.txt']
